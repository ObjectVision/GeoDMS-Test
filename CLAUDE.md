...
- **`-sp`** — run with `/SP` (PerformanceLogging, GeoDMS 20.16.0+). **Off by default**; turned on, GeoDMS writes per-operation cost and footprint plus the memory diagnostics (huge-alloc lines, live-memory census, end-of-run allocation histogram) to the log, and nothing here reads it (GeoDMS #1203).
