...
def get_local_machine_parameters() -> dict:
    ...
    # OVSRV10 — Maarten's dev box. Source data lives on the F: drive;
    # local data uses the (existing) C:/LocalData root.
    local_machine_parameters["GEODMS_OVERRIDABLE_RegressionTestsSourceDataDir"] = "F:/SourceData/RegressionTests"
    local_machine_parameters["RegressionTestsAltSourceDataDir"] = "F:/SourceData"
...
def get_geodms_paths(version:str) -> dict:
    ...
    # OVSRV10: support a "local" pseudo-version that points at the just-built
    # CMake Release in the working tree, so the regression suite can be run
    # without installing an NSIS package.
    if version == "local":
        geodms_paths["GeoDmsPath"] = "\"C:/dev/GeoDMS_2026/build/windows-x64-release/bin\""
    else:
        geodms_paths["GeoDmsPath"] = f"\"{os.path.expandvars(f"%ProgramFiles%/ObjectVision/GeoDms{version}")}\""
    geodms_paths["GeoDmsProfilerPath"] = f'C:/dev/GeoDMS_2026/profiler/profiler.py'
    geodms_paths["GeoDmsRegressionPath"] = f'C:/dev/GeoDMS_2026/profiler/regression.py'
