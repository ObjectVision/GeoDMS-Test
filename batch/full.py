...
def get_experiments(local_machine_parameters:dict, geodms_paths:dict, regression_test_paths:dict, result_paths:dict, version:str, MT1:str, MT2:str, MT3:str) -> list:
    ...
    if _vm and tuple(int(g) for g in _vm.groups()) < (18, 1, 0):
        ...
        # De pre-18.1.0-kopie is een gegenereerd artefact van Operator.dms; genereer
        # hem automatisch (bij) als hij ontbreekt of verouderd is, zodat 17.x-runs
    ...
    # nodig). Eén experiment per familie zodat het rapport rekentijden en
    # correctheids-indicatoren (afwijking t.o.v. geos-referentie) naast elkaar zet.
    regression_test_paths["PolygonComparePath"] = f"{regression_test_paths["TstDir"]}/Polygons/cfg/compare.dms"
    for _fam in ("geos", "bg", "bp", "cgal"):
        regression.add_exp(exps, name=f"{result_folder_name}__t020_polygons_{_fam}", cmd=f"{geodms_paths["GeoDmsRunPath"]} /L{result_paths["results_log_folder"]}/t020_polygons_{_fam}.txt /{MT1} /{MT2} /{MT3} {regression_test_paths["PolygonComparePath"]} results/{_fam}/stored_result", exp_fldr=f"{result_paths["results_folder"]}", env=env_vars, log_fn=f"{result_paths["results_log_folder"]}/t020_polygons_{_fam}.txt", indicator_results_file=f"{result_paths["results_folder"]}/t020_polygons_{_fam}.txt")
    ...
    regression.add_exp(exps, name=f"{result_folder_name}__t200_grid_Poly2Grid", cmd=f"{geodms_paths["GeoDmsRunPath"]} /L{result_paths["results_log_folder"]}/t200_grid_Poly2Grid.txt /{MT1} /{MT2} /{MT3} {regression_test_paths["RegressionPath"]} results/t200_grid_Poly2Grid/result_json", exp_fldr=f"{result_paths["results_folder"]}", env=env_vars, log_fn=f"{result_paths["results_log_folder"]}/t200_grid_Poly2Grid.txt", indicator_results_file=f"{result_paths["results_folder"]}/t200_grid_Poly2Grid.txt") 
    ...
    regression.add_exp(exps, name=f"{result_folder_name}__t300_xml_ReadParse", cmd=f"{geodms_paths["GeoDmsRunPath"]} /L{result_paths["results_log_folder"]}/t300_xml_ReadParse.txt /{MT1} /{MT2} /{MT3} {regression_test_paths["RegressionPath"]} results/t300_xml_ReadParse/result_json", exp_fldr=f"{result_paths["results_folder"]}", env=env_vars, log_fn=f"{result_paths["results_log_folder"]}/t300_xml_ReadParse.txt", indicator_results_file=f"{result_paths["results_folder"]}/t300_xml_ReadParse.txt")
    # t301 — Woningtype (residential type) afleiden uit BAG-geometrie pand/vbo; vergelijk met referentie (1 promille foutmarge).
    regression.add_exp(exps, name=f"{result_folder_name}__t301_BAG_ResidentialType", cmd=f"{geodms_paths["GeoDmsRunPath"]} /L{result_paths["results_log_folder"]}/t301_BAG_ResidentialType.txt /{MT1} /{MT2} /{MT3} {regression_test_paths["RegressionPath"]} results/t301_BAG_ResidentialType/result_json", exp_fldr=f"{result_paths["results_folder"]}", env=env_vars, log_fn=f"{result_paths["results_log_folder"]}/t301_BAG_ResidentialType.txt", indicator_results_file=f"{result_paths["results_folder"]}/t301_BAG_ResidentialType.txt")
    ...
    regression.add_exp(exps, name=f"{result_folder_name}__t720_2BURP", cmd=f"{geodms_paths["GeoDmsRunPath"]} /L{result_paths["results_log_folder"]}/t720_2BURP.txt /{MT1} /{MT2} /{MT3} {regression_test_paths["TwoBURPRunPath"]} t720_2BURP_indicator_results/result_json", exp_fldr=f"{result_paths["results_folder"]}", env=env_vars, log_fn=f"{result_paths["results_log_folder"]}/t720_2BURP.txt")
    ...
    regression.add_exp(exps, name=f"{result_folder_name}__t910_Cusa2_Africa", cmd=f"{geodms_paths["GeoDmsRunPath"]} /L{result_paths["results_log_folder"]}/t910_Cusa2_Africa.txt /{MT1} /{MT2} /{MT3} {regression_test_paths["CusaRunPath"]} t910_cusa2_Africa_test/result_json", exp_fldr=f"{result_paths["results_folder"]}", env=env_vars, log_fn=f"{result_paths["results_log_folder"]}/t910_Cusa2_Africa.txt")
    ...
    if _t1742_maj.isdigit() and int(_t1742_maj) < 18:
    ...
    if geodms_paths.get("GeoDmsLocalFlavor") == "linux-release":
        ...
        for exp in operator_experiments:
            ...
            if exp.environment_variables:
                ...
                # GUI tests (GeoDmsGuiQt) on .l run ON SCREEN via WSLg -- the WSL session
                # provides DISPLAY/WAYLAND_DISPLAY itself; windows appear on the Windows
                # desktop during the run. Do NOT force QT_QPA_PLATFORM=offscreen: offscreen
                # runs hang until the per-test timeout cap (t1630/t1640/t1642 on 20.7.0.l),
                # while the GUI unit tests prove the on-screen WSLg path works fine. (The
                # 20.3.0.l-era display-init crash that once motivated offscreen no longer
                # reproduces.)
