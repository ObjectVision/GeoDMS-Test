...
# Per-test pass/fail policy, applied by the REPORT (GeoDMS only measures). Tune here
# and rerun with -report-only -- no model rerun. Cell metrics: tolerance_pct (default
# 0.0 = exact match required). Scalar metrics: an "expected" value + tolerance_pct.
# Curated here (version-controlled), never regenerated by a test run -- so a real
...
def parse_result_json(path:str, prev_indicators:dict={}) -> tuple:
    """Read a <test>.result.json MEASUREMENT (raw counts / values; GeoDMS does not
    judge) and return the (verdict_text, indicator_dict) shape the rest of the report
    expects. The verdict is computed HERE from the measurements + TEST_TOLERANCES, so
    tolerances are tunable with -report-only. Any verdict/pct/status baked into an older
    JSON is ignored. indicator_dict maps a display line key -> [html, ok]; ok=False
    highlights the line (a failing metric, or drift vs the previous version)."""
    import json
    with open(path, "r", encoding="utf-8") as f:
        doc = json.load(f)
    tol_cfg = TEST_TOLERANCES.get(doc.get("test", ""), {})
    metrics = doc.get("metrics", [])

    lines = {}
    any_fail = any_pending = False
    cmp_metrics = [m for m in metrics if "n_diff" in m]
    totals = {m.get("n_total") for m in cmp_metrics if m.get("n_total") is not None}
    single_total = len(totals) == 1
    if single_total:
        lines["cells in test"] = [f"cells in test: {next(iter(totals))}", True]

    for m in metrics:
        name = str(m.get("name", "metric"))
        cfg = tol_cfg.get(name, {})
        tol = cfg.get("tolerance_pct", 0.0)
        if "n_diff" in m:
            ...
            extra = "" if (single_total or m.get("n_total") is None) else f" of {m.get('n_total')}"
            tol_s = f", tolerance {tol:.2f}%" if tol else ""
            lines[name] = [f"{name} different: {n_diff}{extra} ({pct:.2f}%{tol_s})", ok]
        elif "expected" in cfg:
            value, exp = m.get("value"), cfg["expected"]
            dev = (100.0 * abs(value - exp) / abs(exp)) if exp else (0.0 if value == exp else 100.0)
            ok = dev <= tol
            any_fail = any_fail or not ok
            lines[name] = [f"{name}: {value} {m.get('unit','')} (expected {exp}, {dev:+.2f}%, tolerance {tol:.2f}%)".replace("  ", " "), ok]
        else:  # a measured value with no curated reference yet
            any_pending = True
            lines[name] = [f"{name}: {m.get('value')} {m.get('unit','')} (no reference yet)".replace("  ", " "), True]
